...
import PairOrCard from "../../../components/PairOrCardEn";
...
class IssueAmka extends React.Component {
  constructor(props) {
    ...
    this.proceedWithEidas = this.proceedWithEidas.bind(this);
  ...
  proceedWithEidas() {
    ...
    window.location.href = this.props.baseUrl?`${this.props.baseUrl}eidas/eidas-authenticate${sessionFrag}`:`${this.props.baseUrl}/eidas/eidas-authenticate${sessionFrag}`;
  }

  render() {
    let stepNumber = !this.props.DID ? 0 : this.hasRequiredAttributes ? 2 : 1;
    let stepperSteps = [
      { title: "Pair your SSI Wallet" },
      { title: 'Authenticate over eIDAS eID ' },
      { title: "Issue your Verifiable Credential" },
    ];
    ...
    let amkaLoginButton = !this.hasRequiredAttributes ? (
      <Button onClick={this.proceedWithEidas}>eIDAS eID</Button>
    ) : (
    ...
    let eidasCard = (
      <Card className="text-center" style={{ marginTop: "2rem" }}>
          <Card.Header style={{"fontSize": "1.5rem"}}>Generate a Verifiable Credential attesting to your eID attributes retrieved over the eIDAS Network</Card.Header>
        <Card.Body>
          <Card.Title style={{"fontSize": "1.5rem"}}>
            {this.hasRequiredAttributes
               ? "All required Data have been retrieved!"
               : "Please authenticate over the requested Data Sources"}
          </Card.Title>
          <Card.Text style={{"fontSize": "1.3rem"}}>
          {this.hasRequiredAttributes
               ? `Click now the "Issue" button to receive a Verifiable Credential made of your eID attributes`
               : `Once you complete your eIDAS eID authentication, click the "Issue" button to generate a Verifiable Credential from your eID attributes.
               `}
    ...
    return (
      <Layout text={"UAegean Issuer"}>
        ...
        <Row>
          <HomeButton baseUrl={this.props.baseUrl} text={"Home"} />
        </Row>
      </Layout>
    );
  }
